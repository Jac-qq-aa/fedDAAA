...
def set_seed(seed: int):
    ...
    torch.backends.cudnn.deterministic = True  # 确保cuda的计算是确定性的
    torch.backends.cudnn.benchmark = False  # 禁用自适应算法，以便保持确定性

# 设置种子为固定的值
set_seed(42)  # 你可以根据需要调整种子值


def parameter_alignment(global_model, model, dataloader, lr=0.002, epochs=1):


    local_prototypes = [[] for _ in range(62)]
    global_model.train()
    ...
    alignment_optimizer = torch.optim.SGD(global_model.parameters(),lr=0.005)  # Adjust learning rate and optimizer as needed
        ...
        for x_batch, y_batch in dataloader:
            ...
            for label in y_batch.unique():
                if mean_prototypes[label.item()] is not None:
                        #获取tensor的第一个维度的值 根据这个值 对均值原型参数进行扩展
                        first_vector_length = global_proto_batch[y_batch == label].size(0)
                        prototype_expanded = mean_prototypes[label.item()].unsqueeze(0).expand(first_vector_length, -1)
                        loss += alignment_loss_fn(global_proto_batch[y_batch == label], prototype_expanded)
...
def add_model(global_model, model, dataloader, lr=0.002, epochs=1):
    merged_model = ...
    with torch.no_grad():
        for p1, p2, p_merged in zip(global_model.parameters(), model.parameters(), merged_model.parameters()):
            p_merged.data = 0.5 * p1.data + 0.5 * p2.data
    return model
...
def local_update(model, dataloader, global_model, sigma0, clipping_bound):
    ...
    parameter_alignment(copy_model,model,dataloader)


    #global_model = global_model.to(device)
    fisher_diag , fisher_sum = compute_fisher_diag(model, dataloader)
    w_glob = [param.clone().detach() for param in copy_model.parameters()]
    ...
    total_params = 0
    personalized_params = 0

    for fisher_value in fisher_diag:
        mask = (fisher_value > fisher_threshold)
        personalized_params += mask.sum().item()
        total_params += mask.numel()

    pi = personalized_params / total_params
    print(pi)


    u_glob, v_glob = [], []
    for global_param, fisher_value in zip(copy_model.parameters(), fisher_diag):
    ...
    return new_model, new_clipping_bound,fisher_sum,pi
...
def main():
    ...
    for epoch in trange(global_epoch):
        # 随机采样 sampled_client_indices是客户端序号
        sampled_client_param = []
        ...
        for idx, (client_model, client_trainloader, client_testloader, client_id) in enumerate(
            ...
            noise_client_model, new_client_clipping_bound,fisher_sum,pi = local_update(client_model, client_trainloader, global_model,
                                                                         sigma0,
            ...
            #f(x)=−log(pi+ϵ) 对数放大
            epsilon = 1e-6
            # sampled_client_param.append(-math.log(pi+epsilon))
            r = args.exp
            if (pi != 0):
                sampled_client_param.append(math.log(pi)**r)
            else:
                sampled_client_param.append(pi)
        print(sampled_client_param)
        ...
        sampled_client_weights = [
            sampled_client_data_size / sum(sampled_client_param)
            for sampled_client_data_size in sampled_client_param
        ]
